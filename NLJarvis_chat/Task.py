...
def weather():
    api_key = "cd9b83f298eba06135ab892cd08d4310" #generate your own api key from open weather
...
def InputExecution(tag, query):
    ...
    if "wikipedia" in tag:
        ...
        from NewsRead import latestnews
        latestnews()
    elif "Note" in query or "Note" in tag:
        from doc import Note
        Note()
    elif "email" in query or "email" in tag:
        from outlook import main
        main()
    elif "whatsapp" in query or "whatsapp" in tag:
        from Whatsapp import send_whatsapp_message
        send_whatsapp_message()
    elif "summary" in query or "summary" in tag:
        from summ import text_summarization
        text_summarization()
